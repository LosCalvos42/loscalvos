...
namespace LOSCALVOS
{
    public partial class FrmAddCategoriaMerma : Form
    {
        public FrmAddCategoriaMerma()
        {
            InitializeComponent();
        }
        ...
        private void Inicio()
        {
            ...
            if (Tipo == "MODIFICAR")
            {
                ...
                try
                {
                    lst.Add(new ClsParametros("@listado", "CATEGORIAMERMA"));
                    ...
                    if (dt.Rows.Count > 0)
                    {
                       ...
                       CmbProceso.SelectedValue= dt.Rows[0][5].ToString();
                       TxtPMin.Text= dt.Rows[0][6].ToString();
                       TxtPMax.Text= dt.Rows[0][7].ToString();
        ...
        private bool valido()
        {
            ...
            if (TxtPMin.Text == "")
            {
                MessageBox.Show("El Campo Porcentaje Minimo de merma NO puede estar Vacio", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                TxtDescripcion.Focus();
                return false;
            }
            if (TxtPMax.Text == "")
            {
                MessageBox.Show("El Campo Porcentaje maximo de merma NO puede estar Vacio", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                TxtDescripcion.Focus();
                return false;
            }
            if (CmbProceso.SelectedIndex == 0)
            {
                MessageBox.Show("Debe Seleccionar un Proceso", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                CmbProceso.Focus();
                return false;
            }
        ...
        private String[] AbmUser(string tipo)
        {
            ...
            try
            {
                ...
                lst.Add(new ClsParametros("@PMIN", Convert.ToDouble(TxtPMin.Text)));
                lst.Add(new ClsParametros("@PMAX", Convert.ToDouble(TxtPMax.Text)));
                ...
                M.EjecutarSP("sp_AddCategoriaMerma", ref lst);
                msj = new string[2];
