...
namespace LOSCALVOS
{
    ...
    {
        public FrmAddOrdenDeTrabajoEstucado()
        {
            InitializeComponent();
        }
        ...
        private void FrmAddGrupoFamilia_Load(object sender, EventArgs e)
        {
            ...
            Cargarcombo("PROCESOELABMERMA", CmbProceso);
            Inicio();
        }
        ...
        private void Inicio()
        {
            ...
            if (Tipo == "NUEVO")
            {
                LblTitulo.Text = "Orden De Trabajo De Elaboracion Por Mermas";
                id = 0;              
                TxtObservacion.ReadOnly = false;
                groupBox1.Enabled = true;
                btnAceptar.Enabled = true;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        GetcategoriaMerma(Convert.ToDecimal(Dgprincipal.Rows[i].Cells[4].Value), CmbProceso.SelectedValue.ToString());
        ...
        private void Registrar(string codbar , int fila)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                Dgprincipal.Rows[fila].Cells[6].Value=GetcategoriaMerma (merma,CmbProceso.SelectedValue.ToString()) ;
                ...
                string[] msg = ABMPROCESO(codbar, Dgprincipal.Rows[fila].Cells[7].Value.ToString(), Pesodecimal, dt.Rows[0][2].ToString(), Dgprincipal.Rows[fila].Cells[6].Value.ToString());
                GetLoteDeOt();
                if (msg[0] == "0")
                {
        ...
        private String[] ABMPROCESO(string CODBAR,string LOTE,decimal PESO, string CatPeso,string CatMerma)
        {
            ...
            try
            {
                ...
                lst.Add(new ClsParametros("@CATEGORIAMERMA ", CatMerma));
                lst.Add(new ClsParametros("@ALMACEN", CmbAlmacen.SelectedValue));
                lst.Add(new ClsParametros("@ESTADOCODBAR", EstadoCodbar));
                lst.Add(new ClsParametros("@USR_ID", Program.IDUSER));
                lst.Add(new ClsParametros("@Resultado", "", SqlDbType.VarChar, ParameterDirection.Output, 5));
        ...
        private void Calculo_mermasMinMax()
        {
            ...
            for (int i = 0; i < Dgprincipal.Rows.Count-1; i++)
            {
                ...
                TotalKg = TotalKg + Convert.ToDecimal(Dgprincipal.Rows[i].Cells[3].Value);
            }
            TxtMermaMin.Text = Convert.ToString(mermamin);
            TxtMermaMaxima.Text = Convert.ToString(mermamax);
            TxtPesoProceso.Text = Convert.ToString(TotalKg);
            Txtpiezas.Text = Convert.ToString(Dgprincipal.Rows.Count - 1);

        }
        private string GetcategoriaMerma(decimal peso,string proceso)
        {
            ClsManejador M = new ClsManejador();
            DataTable dt = new DataTable();
            List<ClsParametros> lst = new List<ClsParametros>();

            lst.Add(new ClsParametros("@merma", peso));
            lst.Add(new ClsParametros("@codProceso", proceso));

            dt = M.Listado("SP_GetCategoriaMerma", lst);

            if (dt.Rows.Count > 0)
            {
                TxtMminD.Text = dt.Rows[0][1].ToString();
                TxtMMaxD.Text = dt.Rows[0][2].ToString();
        ...
        private void Getinfo(int fila)
        {
            ...
            { 
                ...
                Dgprincipal.Rows[fila].Cells[6].Value = GetcategoriaMerma(Convert.ToDecimal(Dgprincipal.Rows[fila].Cells[4].Value), CmbProceso.SelectedValue.ToString());
                ...
                LblLetra.Text = Dgprincipal.Rows[fila].Cells[6].Value.ToString();
        ...
        private void GetLoteDeOt()
        {
            DataTable dt = new DataTable();
            ClsManejador M = new ClsManejador();
            string ssql = @"select distinct(p.LOTE) from MOVIMIENTOS m
                            inner join PRODUCCION p on p.PRODUCCION_ID= m.PRODUCCION_ID
                            where m.MOVIMIENTOS_NROOT="+ Convert.ToUInt64(TxtNroOT.Text);
            dt = M.lisquery(ssql);

            TxtObservacion.Text = "Lotes:";
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                TxtObservacion.Text= TxtObservacion.Text+" "+ dt.Rows[0][0].ToString() +" -";
            }
        ...
        private void BtnRacks_Click(object sender, EventArgs e)
        {
            if (Dgprincipal.Rows.Count > 1)
            {
                ...
                FrmImpresionPallet _FrmImpresionPallet = new FrmImpresionPallet();
                _FrmImpresionPallet.Categoria = Dgprincipal.Rows[0].Cells[6].Value.ToString();
                _FrmImpresionPallet.NROOT = Convert.ToInt32(TxtNroOT.Text);
                _FrmImpresionPallet.Tipo = tipoimp;
                _FrmImpresionPallet.Lote = lotes;
                _FrmImpresionPallet.RackCodbar = "";
                _FrmImpresionPallet.StartPosition = FormStartPosition.CenterScreen;
                _FrmImpresionPallet.ShowDialog();
                tipoimp = 0;
            }
        }
